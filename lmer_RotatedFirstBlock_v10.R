...
#gender phrasing timing
items_ratings_full <- rating_data_long_omit %>%
...
jitter <- ggplot(rating_data_long_omit, aes(x = Valence, y = Arousal, color = Timing)) +
  geom_jitter(width = .5, height = .5) +
  geom_smooth(method = "lm")
ggExtra::ggMarginal(jitter, type = "histogram", fill = "light blue")
...
#scatter by timing 
itemscattertiming <- ggscatter(items_ratings_full, x = "Valence", y = "Arousal",
                            color = "Timing",
                            add = "reg.line", conf.int = TRUE, xlim=c(0,7), ylim =c(0,7))
itemscattertiming + stat_cor(aes(color = Timing), label.x = 6)
ggExtra::ggMarginal(itemscatterSex, type = "histogram", fill = "light blue")

ggboxplot(items_ratings, x = "Timing", y = "Valence", width = 0.3, ylim=c(0,7))
...
#scatter by sex 
itemscatterSex <- ggscatter(items_ratings_full, x = "Valence", y = "Arousal",
                         color = "Sex",
...
levels(items_ratings_full$Phrasing)[levels(items_ratings_full$Phrasing)=="1"] <- "FeelingRating"
levels(items_ratings_full$Phrasing)[levels(items_ratings_full$Phrasing)=="0"] <- "ImageRating"
#scatter by phrasing
itemscatterPhrasing <- ggscatter(items_ratings_full, x = "Valence", y = "Arousal",
                            color = "Phrasing",
                            add = "reg.line", conf.int = TRUE, xlim=c(0,7), ylim =c(0,7))
itemscatterPhrasing + stat_cor(aes(color = Phrasing), label.x = 6)
ggExtra::ggMarginal(itemscatterSex, type = "histogram", fill = "light blue")
...
ggscatterhist(
  items_ratings_full, x = "Valence", y = "Arousal",
...
#valence timing bar
library(Rmisc)
SEsum_timing <- summarySE(items_ratings_full, measurevar="Valence", groupvars=c("Timing"))
SEsum_timing$Timing <- as.factor(SEsum_timing$Timing)
ggplot(SEsum_timing, aes(x=Timing, y=Valence, fill=Timing)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=Valence-SEsum_timing$ci, ymax=Valence+SEsum_timing$ci),
                width=.08,                    # Width of the error bars
                position=position_dodge(.9))

#valence phrasing bar
library(Rmisc)
phrasing <- summarySE(items_ratings_full, measurevar="Valence", groupvars=c("Phrasing"))
phrasing$Timing <- as.factor(phrasing$Phrasing)
ggplot(phrasing, aes(x=Phrasing, y=Valence, fill=Phrasing)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=Valence-phrasing$ci, ymax=Valence+phrasing$ci),
                width=.08,                    # Width of the error bars
                position=position_dodge(.9))

#arousal timing bar
SEsum_timing_arousal <- summarySE(items_ratings_full, measurevar="Arousal", groupvars=c("Timing"))
SEsum_timing_arousal$Timing <- as.factor(SEsum_timing_arousal$Timing)
ggplot(SEsum_timing_arousal, aes(x=Timing, y=Arousal, fill=Timing)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=Arousal-SEsum_timing_arousal$ci, ymax=Arousal+SEsum_timing_arousal$ci),
...
#valence sex bar
library(Rmisc)
SEsum <- summarySE(items_ratings_gender, measurevar="Valence", groupvars=c("Timing","Sex", "Phrasing"))
SEsum$Timing <- as.factor(SEsum$Timing)
ggplot(SEsum, aes(x=Timing, y=Valence, fill=Sex)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=Valence-SEsum$ci, ymax=Valence+SEsum$ci),
                width=.2,                    # Width of the error bars
                position=position_dodge(.9))

SEsum <- summarySE(items_ratings_gender, measurevar="Arousal", groupvars=c("Timing","Sex"))
SEsum$Timing <- as.factor(SEsum$Timing)
ggplot(SEsum, aes(x=Timing, y=Arousal, fill=Sex)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=Arousal-SEsum$ci, ymax=Arousal+SEsum$ci),
                width=.2,                    # Width of the error bars
                position=position_dodge(.9))

ggplot(data=items_ratings_full, aes(x=Timing, y=Valence, fill=Sex)) +
  geom_bar(stat="identity", position=position_dodge(), xlim=c(0,7))
...
item_phrasing <- ggscatter(items_ratings_phrasing, x = ("Valence"), y = ("Arousal"),
                             color = "Phrasing",
                             add = "reg.line", conf.int = TRUE, xlim=c(0,8), ylim =c(0,8))
item_phrasing + stat_cor(aes(color = Phrasing), label.x = 6)
ggExtra::ggMarginal(item_phrasing, type = "histogram", fill = "light blue")


ggscatterhist(
  items_ratings_phrasing, x = "Valence", y = "Arousal",
  color = "Phrasing", size = 3, alpha = 0.6,
  add = "reg.line", conf.int = TRUE,
  palette = c("Dark3"),
  margin.plot = "boxplot",
  xlim=c(0,7),
...
#scatter 
#PCA
itemscattertiming <- ggscatter(RCA_ratings, x = "RC1", y = "RC2",
                               color = "Timing",
                               add = "reg.line", conf.int = TRUE)
itemscattertiming + stat_cor(aes(color = Timing), label.x = 6)
...
RCA_ratings <- fit_FinalDataFrame %>%
  group_by(Item, Timing, Phrasing, Sex) %>%
...
library(ggplot2)
RCA_item_scatter_phrasing <- ggplot(RCA_ratings, aes(x = RC1, y = RC2)) +
  geom_point() + aes(color = Phrasing, add = "reg.line", conf.int = TRUE) +
  geom_smooth(method = "lm")
RCA_item_scatter_phrasing + theme_classic()

RCAscatter <- ggscatter(RCA_ratings, x = "RC1", y = "RC2",
                     color = "Timing",
                     add = "reg.line", conf.int = TRUE)
RCAscatter + stat_cor(aes(color = Timing), label.x = .03)

ggscatterhist(
  RCA_ratings, x = "RC1", y = "RC2",
  color = "Timing", size = 3, alpha = 0.6,
  add = "reg.line", conf.int = TRUE,
  palette = c("Dark3"),
  margin.plot = "boxplot",
  xlim=c(-1,1),
...
#timing_bars
library(Rmisc)
RCA_timing <- summarySE(RCA_ratings, measurevar="RC2", groupvars=c("Timing", "Phrasing"))
RCA_timing$Timing <- as.factor(RCA_timing$Timing)
ggplot(RCA_timing, aes(x=Timing, y=RC2, fill=Phrasing)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=RC2-RCA_timing$ci, ymax=RC2+RCA_timing$ci),
                width=.1,                    # Width of the error bars
                position=position_dodge(.9))

levels(RCA_ratings$Phrasing)[levels(RCA_ratings$Phrasing)=="1"] <- "FeelingRating"
levels(RCA_ratings$Phrasing)[levels(RCA_ratings$Phrasing)=="0"] <- "ImageRating"

#phrasing bars
RCA_phrasing <- summarySE(RCA_ratings, measurevar="RC2", groupvars=c("Phrasing"))
RCA_phrasing$Phrasing <- as.factor(RCA_phrasing$Phrasing)
ggplot(RCA_phrasing, aes(x=Phrasing, y=RC2, fill=Phrasing)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=RC2-RCA_phrasing$ci, ymax=RC2+RCA_phrasing$ci),
                width=.1,                    # Width of the error bars
                position=position_dodge(.9))
...
#*ROTATED
#*FULL DATA W PERS####
RC_PCAmodel_pers <- lmer(RC2 ~ Timing * Sex * Phrasing + STAI_T + ERQ_Cog + ERQ_Exprs + RRQ_Rum + RRQ_Ref + BEQ_Pos + BEQ_Neg + BEQ_Imp + (1|Internal_ID) + (1|Item), data = fit_FinalDataFrame, REML=FALSE) 
summary(RC_PCAmodel_pers) 
#F.value  Pr(>F)  
#Timing 1.8807  1.8807     1 97.993  3.8514 0.05254 .

#no pers
RC_PCAmodel <- lmer(RC2 ~ Timing * Phrasing * Sex + (1|Internal_ID) + (1|Item), data = fit_FinalDataFrame, REML=FALSE) 
summary(RC_PCAmodel) 

#ITEMS LEVEL####[PRESENTED]
ratings_RC_PCAmodel <- lmer(RC2 ~ Timing * Phrasing * Sex + (1|Item), data = RCA_ratings, REML=FALSE) 
...
#pers
ratings_RC_PCAmodel_pers <- lmer(RC2 ~ Timing + Phrasing + Sex + STAI_T + ERQ_Cog + ERQ_Exprs + RRQ_Rum + RRQ_Ref + BEQ_Pos + BEQ_Neg + BEQ_Imp + (1|Item), data = RCA_ratings, REML=FALSE) 
summary(ratings_RC_PCAmodel_pers) 
sjt.lmer(ratings_RC_PCAmodel)  
anova(ratings_RC_PCAmodel_pers)
library(car)
Anova(ratings_RC_PCAmodel_pers, type = "III")
...
#*FULL DATA####
#arousal
arousal_results <- lmer(Arousal ~ Timing * Phrasing * Sex + (1|Internal_ID) + (1|Item), data=rating_data_long) #
...
#valence
valence_results <- lmer(Valence ~ Timing * Phrasing * Sex + (1|Internal_ID) + (1|Item), data=rating_data_long) #
summary(valence_results)

#*ITEMS####
#arosal
ratings_arousal_results <- lmer(Arousal ~ Timing * Phrasing * Sex + (1|Item), data=items_ratings_full) #
...
#valence
ratings_valence_results <- lmer(Valence ~ Timing * Phrasing * Sex + (1|Item), data=items_ratings_full) #
...
fviz + theme_minimal() 
#+ geom_text(aes(label = forclus_2$Valence_allsubjects_standard))

fviz_2_standard <- fviz_cluster(item_2, geom = NULL, data = forclus_2[,c("Valence_allsubjects_study_2","Arousal_allsubjects_study_2")]) + geom_text(aes(label = forclus_2$Valence_allsubjects_standard, color = cluster))
fviz_2_standard + theme_minimal() 
...
#standard valence
fviz_7 <- fviz_cluster(item_7, geom = NULL, data = forclus_7[,c("Valence_allsubjects_study_7","Arousal_allsubjects_study_7")]) + geom_text(aes(label = forclus_7$Valence_allsubjects_standard, color = cluster))
fviz_7 + theme_minimal()
