...
NA -> meta[which(meta == "0.00")]
forclus<-meta[,3:32]
str(forclus)
hdist <- Dist(forclus, method="correlation", upper=T)
plot(hdist)
forclusfit <- Mclust(meta[,c(3,9,19,21,31)])
t(forclusfit$parameters$mean)
dens <- densityMclust(meta[,3:5])
dens$density
summary(densityMclust(meta))
plot(forclusfit, what="uncertainty")
plot(forclusfit, what="BIC") 
plot(forclusfit, what="classification")
table(forclusfit$classification)
clPairs(forclus, classification = forclusfit$classification, symbols = 1:5)


print(cloud(Valence_allsubjects_standard ~ Valence_allsubjects_study_2* Valence_allsubjects_diff_2, data=meta, col=meta$Item, perspective=T, distance=0.4, type="p", shade=T, lex=2, xlab="Valence_standard", ylab="Valence_study", zlab="Valence_diff", main="3D Scatterplot", R.mat = diag(4), screen = list(z=-40, x=-42), aspect = c(1,1)),  position = c(0, 0, 0.5, 1), more = TRUE)
